"""
Script de données de démonstration — enoschofficiel
============================================
...
def creer_user(username, password, first_name, last_name, email, role, is_staff=False):
    ...
    profile.role = role
    profile.bio = f"{first_name} {last_name} – rédacteur pour enoschofficiel."
    profile.save()
    return user, created
...
auteur, c = creer_user(
    "marie_journaliste", "Demo1234!", "Marie", "Joseph", "marie@lemedia.ht", "auteur"
)
ok(
    f"Auteur           : marie_journaliste / Demo1234!  {'(nouveau)' if c else '(existant)'}"
)

editeur, c = creer_user(
    "jean_editeur", "Demo1234!", "Jean", "Pierre", "jean@lemedia.ht", "editeur"
)
ok(f"Éditeur          : jean_editeur / Demo1234!  {'(nouveau)' if c else '(existant)'}")

lecteur, c = creer_user(
    "paul_lecteur", "Demo1234!", "Paul", "Dumont", "paul@lemedia.ht", "lecteur"
)
...
<p>Les premières estimations font état de <strong>plusieurs milliers de familles sinistrées</strong>, de routes coupées et de ponts endommagés. La Protection Civile coordonne les opérations de secours avec l'appui des forces armées.</p>""",
    ...
    (
        ...
        12,
        ["haïti", "football"],
        """<p>Rencontré en exclusivité par enoschofficiel, le sélectionneur de l'équipe nationale de football revient sur la qualification pour la Gold Cup et dévoile ses ambitions pour les prochaines années.</p>
...
MESSAGES = [
    (
        ...
        "sophie@gmail.com",
        "redaction",
        "Bonjour, je souhaite proposer un reportage sur la situation des femmes entrepreneures en Haïti. Serait-il possible d'en discuter avec votre équipe ?",
    ),
    (
        "Marc Antoine",
        "marc.antoine@yahoo.com",
        "correction",
        "Dans votre article sur les élections, il y a une erreur de date. Le scrutin est prévu en juillet et non en juin comme indiqué. Merci de corriger.",
    ),
    (
        "Entreprise XY",
        "contact@entreprise.ht",
        "publicite",
        "Notre groupe est intéressé par des opportunités publicitaires sur votre plateforme. Pourriez-vous nous envoyer votre grille tarifaire ?",
    ),
    (
        "Citoyen Anonyme",
        "citoyen@proton.me",
        "general",
        "Félicitations pour la qualité de votre couverture journalistique. enoschofficiel est devenu ma référence pour l'information en Haïti. Continuez ainsi !",
    ),
    (
        "ONG Haïti Demain",
        "info@haitidemain.org",
        "partenariat",
        "Notre organisation souhaite explorer un partenariat médiatique pour la couverture de nos projets de développement. Serait-il possible d'organiser une réunion ?",
    ),
]
